...
{
    if ( lv_pm_router[id]){
        return lv_pm_router[id];
    }
  // lv_pm_page_t *pm_page = (lv_pm_page_t *)lv_mem_alloc(sizeof(lv_pm_page_t));
  auto *pm_page = (lv_pm_page_t *)lv_mem_alloc(sizeof(lv_pm_page_t));
  if (pm_page == NULL) {
    fmt::newline_info("lv_pm_page is null");
    return NULL;
  }
...
uint8_t lv_pm_back()
{
    fmt::newline_info("pm_history:{}",lv_pm_history_len);
