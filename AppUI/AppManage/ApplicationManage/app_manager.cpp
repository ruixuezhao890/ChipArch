...
APP_BASE* APP_Manager::createApp(APP_PACKER_BASE* appPacker)
{
    ...
    auto page_packer=(SCREEN_PACKER_BASE *)appPacker->newScreenPacker();
    page_packer->setAppBase(new_app);
    if (page_packer){
        _screen_controller.creatPage(page_packer);
    }
    new_app->setScreenPacker(page_packer);
...
void APP_Manager::update()
{
    ...
    for (auto iter = _app_lifecycle_list.begin(); iter != _app_lifecycle_list.end();)
    {
        ...
        case ON_PAUSE:
            iter->app->onPause();
            ScreenController::backScreen(iter->app->getScreenPacker());
        ...
        case ON_DESTROY:
            ...
            ScreenController::backScreen(iter->app->getScreenPacker());
            iter->app->onDestroy();
            iter->app->getScreenPacker()->deletePage(iter->app->getScreenPacker()->getScreenBase());
            iter->app->getAppPacker()->deleteScreenPacker(iter->app->getScreenPacker());
