...
//        static lv_color_t buf_1[MY_DISP_HOR_RES * 10];
        lv_disp_draw_buf_init(&(this->lv_disp_buf), lv_buf_1.data(), nullptr, fb_size);
#elif BUFFER_NUM==2
        this->lv_buf_1 = std::vector<lv_color_t>(fb_size);
        this->lv_buf_2 = std::vector<lv_color_t>(fb_size);
        lv_disp_draw_buf_init(&(this->lv_disp_buf), this->lv_buf_1.data(),  this->lv_buf_2.data(), fb_size);
#elif BUFFER_NUM==3
       this->lv_buf_1 = std::vector<lv_color_t>(MY_DISP_HOR_RES*MY_DISP_VER_RES);
       this->lv_buf_2 = std::vector<lv_color_t>(MY_DISP_HOR_RES*MY_DISP_VER_RES);
       lv_disp_draw_buf_init(&(this->lv_disp_buf), this->lv_buf_1.data(),  this->lv_buf_2.data(), MY_DISP_HOR_RES*MY_DISP_VER_RES);
#endif
    ...
    void scr_load_anim(Object &scr, lv_scr_load_anim_t anim_type, uint32_t time, uint32_t delay, bool auto_del) {
        lv_scr_load_anim(scr.raw_ptr(),anim_type,time,delay,auto_del);
    }
